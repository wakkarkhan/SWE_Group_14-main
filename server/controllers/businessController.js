...
/**
 * Create a review for business by its ID.
 * @param {Object} req - The request object, containing the business ID.
 * @param {Object} res - The response object used to send responses to the client.
 */
exports.createReviewForBusiness = async (req, res) => {
  const businessId = req.params.id; // Get the business ID from the URL parameters
  const { reviewerId, rating, comment } = req.body; // Destructure the request body

  try {
    // Check if a review from the same reviewer for the same business already exists
    const existingReview = await Review.findOne({ reviewerId, businessId });

    if (existingReview) {
      // If a review already exists, respond with a message
      return res.status(400).send({ isSuccess: false, message: 'You have already reviewed this business' });
    }
    // Create a new review object including the businessId
    ...
    res.status(201).send({ isSuccess: true, data: review });
  } catch (error) {
...
/**
  Get  reviews for business by its ID.
 * @param {Object} req - The request object, containing the business ID.
 * @param {Object} res - The response object used to send responses to the client.
 */
exports.getReviewsForBusiness = async (req, res) => {
  ...
  try {
    // Find all reviews for the specified business
    const reviews = await Review.find({ businessId })
      .populate({
        path: 'reviewerId', // The field to populate
        select: 'username', // Specify which fields you want to retrieve from the User model
      });

    // Transform the reviews to rename reviewerId to reviewer
    const transformedReviews = reviews.map(review => {
      return {
        ...review._doc, // Spread the original review document
        reviewer: review.reviewerId, // Rename reviewerId to reviewer
        reviewerId: undefined // Remove the old reviewerId field
      };
    });


    // Respond with the list of reviews
    res.status(200).send({ isSuccess: true, data: transformedReviews  });
  } catch (error) {
